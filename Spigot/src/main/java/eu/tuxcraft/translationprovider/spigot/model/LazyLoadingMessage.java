...
public class LazyLoadingMessage implements Message {
	...
	@Override public Component getComponentFor(CommandSender cs, boolean isMiniMessage) {
		if (isMiniMessage) {
	...
	@Override public Component getComponentFor(CommandSender cs, Map<String, String> params, boolean isMiniMessage) {
		if (cs instanceof Player player) {
			if (isMiniMessage) {
				return MiniMessage.miniMessage().deserialize(engine.getTranslationForUser(player.getUniqueId(), key, params));
			} else {
				return Component.text(engine.getTranslationForUser(player.getUniqueId(), key, params));
			}
		} else {
			if (isMiniMessage) {
				return MiniMessage.miniMessage().deserialize(engine.getTranslationForUser(null, key, params));
			} else {
